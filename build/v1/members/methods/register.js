...
function default_1(memberRepo, config) {
    return function (req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // perform
            const existingEmail = yield memberRepo.findOne({
                select: ["email"],
                where: {
                    email: es.arguements.email
                }
            });
            const existingUsername = yield memberRepo.findOne({
                select: ["username"],
                where: {
                    username: es.arguements.username
                }
            });
            const existingName = yield memberRepo.findOne({
            ...
            if (existingEmail) {
                ...
                const user = yield memberRepo.create(es.arguements);
                yield memberRepo.save(user)
                    .then((data) => {
