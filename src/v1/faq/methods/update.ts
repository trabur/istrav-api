...
export default function (faqRepo: any, appRepo: any, config: any) {
  return async function (req: Request, res: Response) {
    ...
    // perform
    const object = await faqRepo.findOne({
      where: {
        appId: es.arguements.appId,
        slug: es.arguements.slug
      }
    })
    faqRepo.merge(object, es.arguements.change)
    await faqRepo.save(object)
      .then((data: any) => {
